...
for a in lines1:
    steps_b=0
    for b in [l for l in lines2]:  # only check horizonals vs verticals and not first pair
        if a[0]=='H':
            h=a
            v=b
        else:
        ...
        if (h[1][0] <= i[0] <= h[2][0] or h[1][0] >= i[0] >= h[2][0]) and (v[1][1] <= i[1] <= v[2][1] or v[1][1] >= i[1] >= v[2][1])\
                and b[0] != a[0] and not (b == lines2[0] and a == lines1[0]):
            stepsums.append(steps_a+steps_b+abs(h[1][0]-v[1][0]) +abs(v[1][1]-h[1][1]) )
        steps_b = steps_b + abs(b[1][0] - b[2][0]) + abs(b[1][1] - b[2][1])
    steps_a = steps_a + abs(a[1][0] - a[2][0]) + abs(a[1][1] - a[2][1])
