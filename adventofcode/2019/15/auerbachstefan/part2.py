    ...
    for j in range(miny,maxy+1):
        for i in range(minx, maxx+1):
            if (i,j) in grid:
                ...
                if grid[(i,j)]=='oxygenbubble':
                    pygame.draw.circle(gameDisplay, blue, (i*10+5+400, j*10+5+300), 2, 1)
...
while True:
    i = get_dir()
    if i == None:
        print('oxy starts flowing...')
        minutes=0
        while True:
            print(oxy)
            print([grid[o] for o in oxy])
            newoxy=[]
            for p in oxy:
                if grid[(p[0]+1, p[1])]=='explored':
                    grid[(p[0]+1, p[1])]='oxygenbubble'
                    newoxy.append((p[0]+1, p[1]))
                if grid[(p[0] - 1, p[1])] == 'explored':
                    grid[(p[0] - 1, p[1])] = 'oxygenbubble'
                    newoxy.append((p[0] - 1, p[1]))
                if grid[(p[0] , p[1]+1)] == 'explored':
                    grid[(p[0] , p[1]+1)] = 'oxygenbubble'
                    newoxy.append((p[0] , p[1]+1))
                if grid[(p[0] , p[1]-1)] == 'explored':
                    grid[(p[0] , p[1]-1)] = 'oxygenbubble'
                    newoxy.append((p[0] , p[1]-1))
            print_grid()
            ...
            if not oxy: print('no more points..')
    ...
    if res == 0:
        ...
        y=y_old
        steps -=1
    #draws only some frames for the robot to walk faster
    if random.randint(1,1)==1: print_grid()
